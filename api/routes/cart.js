...
router.get("/cart/:userId", [authMiddleWare], async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      "SELECT carts.id AS id,user_id,products_id,name,category,img,description,price FROM carts JOIN products ON carts.products_id=products.id WHERE user_id=$1",
      [userId]
    );
